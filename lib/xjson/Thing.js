...
XJSON.Thing.prototype.getReferenceString = function () {
    ...
    if (self.hasOwnProperty('_cachedRefString')) {
        return self._cachedRefString;
    }
    ...
    self._cachedRefString = result;
    return result;
};
XJSON.Thing.prototype.getStyles = function () {
    "use strict";
    var self = this,
        myRefString = self.getReferenceString();
    if (self.root.stylesheet.hasOwnProperty(myRefString)) {
        return self.root.stylesheet[myRefString];
    } else {
        return {};
    }
};
XJSON.Thing.prototype.setStyles = function (styles) {
    "use strict";
    var self = this,
        myRefString = self.getReferenceString(),
        existingStyles = {},
    ...
    self.root.stylesheet[myRefString] = styles;
    if (self.root.stylesheet.hasOwnProperty(myRefString)) {
...
XJSON.Thing.prototype.getStyle = function (styleName, defaultValue) {
    ...
    var self = this,
        myRefString = self.getReferenceString(),
        result;
    if (self.root.stylesheet.hasOwnProperty(myRefString) === false) {
        self.root.stylesheet[myRefString] = {};
    }
    if (self.root.stylesheet[myRefString].hasOwnProperty(styleName) === false) {
        self.root.stylesheet[myRefString][styleName] = defaultValue;
    }
    return self.root.stylesheet[myRefString][styleName];
};
